const PROJECT_ID = "accessible-living-database";
const DATABASE_ID = "accessible-living-test";
...
// Equivalent to setDoc(..., { merge: true }) — only the named fields are written.
...
// Firestore doc IDs cannot contain "/". Sanitize normalized address keys.
...
// Extracts the 5-digit ZIP code from an address string.
// Tries end-of-string first; falls back to the first 5-digit number after a
// comma, so trailing noise doesn't block the match and building numbers
// (which precede any comma) are never mistaken for a ZIP.
...
// Extracts the city from a comma-separated address like "123 Main St, Springfield, IL 62701"
// Returns lowercase so Firestore equality queries are case-insensitive
function parseCity(address) {
  const parts = address.split(",").map(s => s.trim());
  // City is the second-to-last comma segment (before "State ZIP")
      ...
      chrome.scripting.executeScript(
        {
          ...
          func: () => {
            ...
            try {
              ...
              if (!result.address) {
                // Matches addresses with a street-type suffix OR a state+zip at the end
                const addrPattern =
                  /\d{1,5}\s+\S.{2,50}?(?:st|street|ave|avenue|rd|road|dr|drive|ln|lane|blvd|boulevard|way|ct|court|pkwy|hwy|[A-Z]{2}\s+\d{5})/i;
                // Search headings first, then any element with "address" in class/id
                const candidates = [
                  ...document.querySelectorAll("h1, h2, h3"),
                  ...document.querySelectorAll("[class*='address' i], [id*='address' i]"),
                ];
                const maybe = candidates.find((el) =>
                  addrPattern.test((el.innerText || el.textContent) || "")
                );
                if (maybe) {
                  result.address = (maybe.innerText || maybe.textContent)
                    .replace(/\s+/g, " ")
                    .trim();
                }
              }
        ...
        (results) => {
          if (chrome.runtime.lastError || !results || !results.length) { resolve(null); return; }
...
async function handleSave() {
  ...
  try {
    ...
    // Parse zip and city so the website can query them directly without fetching all documents
    ...
    await firestoreSet(`reports/${key}`, {
      address, apartment, key, zip, city,
      ...featureFields,
      updatedAt: savedAt,
...
function renderRecord(record) {
  ...
  document.querySelectorAll(".accessibility-checkbox").forEach((cb) => {
    cb.checked = Array.isArray(record.accessibilityFeatures)
      ? record.accessibilityFeatures.includes(cb.value)
      : false;
  });
...
async function handleLoad() {
  ...
  try {
    ...
    if (!parent) {
      setStatus(
        apartment
          ? "No report found for this address and apartment."
          : "No report found for this address yet.",
        "error"
      );
      setMeta("");
      clearHistory();
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (info) {
      ...
      setStatus(
        info.address
          ? "Pre-filled from the current page. Edit before saving if needed."
          : "Could not auto-detect listing info. Enter it manually.",
